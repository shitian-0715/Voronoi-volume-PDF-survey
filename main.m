...
a26 = 3.317; b26 = 3.0368;  c26 = 1.0787;
...
% guard_ratio = 0.3;
% 
% valid_areas = areas(A_xmin,A_xmax,A_ymin,A_ymax, guard_ratio);
% 
...
% plotStyled(ax1_tail, x3,     y3,     'Weaire',1.0);
% plotStyled(ax1_tail, x4,     y4,     'DiCenzo',1.0);
% plotStyled(ax1_tail, x5,     y5,     'Ferenc',1.0);
% plotStyled(ax1_tail, x1_2d,  y1_2d,  'Prop.(2 param.)',1.0);
% plotStyled(ax1_tail, x2_2d,  y2_2d,  'Prop.(3 param.)',1.0);
% 
% % ====== 设置拖尾放大范围（右尾） ======
% xlim(ax1_tail,[4.858 4.876]);
% ylim(ax1_tail,[0.2e-4 0.8e-4]);
% 
...
% w_tail2 = 0.42 * pos2(3);
% 
% x_tail2 = pos1(1) + 0.50 * pos2(3);
% y_tail2 = pos1(2) + 0.08 * pos2(4);    % 在峰值 inset 上方
% 
...
% plotStyled(ax2_tail, x8,     y8,     'Ferenc',1.0);
% plotStyled(ax2_tail, x_3d,   y_3d,   'Prop.(2 param.)',1.0);
% plotStyled(ax2_tail, x2_3d,  y2_3d,  'Prop.(3 param.)',1.0);
% 
% % ====== 拖尾范围 ======
% xlim(ax2_tail,[3.6603 3.6605]);
% ylim(ax2_tail,[0.5e-4 3e-4]);
% 
% 
...
figC = figure('Units','centimeters','Position',[2 2 figW figH]);
set(figC,'PaperUnits','centimeters','PaperPosition',[0 0 figW figH]);

axC = axes('Parent',figC); hold(axC,'on'); box(axC,'on'); grid(axC,'on');

% 经验 PDF
scatter(axC, bin_centers1, counts1, msz, ...
    'MarkerFaceColor','none', ...
    'MarkerEdgeColor',[0 0 1], ...
    'DisplayName','Empirical');

% 理论曲线
plotStyled(axC, x1,     y1,     'Hinde and Miles', lw);
plotStyled(axC, x2,     y2,     'Tanemura',        lw);
plotStyled(axC, x3,     y3,     'Weaire et al',          lw);
plotStyled(axC, x4,     y4,     'DiCenzo and Wertheim',         lw);
plotStyled(axC, x5,     y5,     'Ferenc and Neda',          lw);
plotStyled(axC, x1_2d,  y1_2d,  'Proposed (TG)', lw);
plotStyled(axC, x2_2d,  y2_2d,  'Proposed (GG)', lw);

% ===== 关键：真正尾部区域 =====
xlim(axC,[2 15]);        % 扩大到渐近区
ylim(axC,[1e-8 1e-2]);  % 用 log 轴

set(axC,'YScale','log');   % 对数坐标
xlabel(axC,'$v$','Interpreter','latex','FontSize',fs_label);
ylabel(axC,'$f(v)$','Interpreter','latex','FontSize',fs_label);
set(axC,'TickDir','in','LineWidth',1);
axC.GridAlpha = 0.15;
...
figD = figure('Units','centimeters','Position',[2 2 figW figH]);
set(figD,'PaperUnits','centimeters','PaperPosition',[0 0 figW figH]);

axD = axes('Parent',figD); hold(axD,'on'); box(axD,'on'); grid(axD,'on');

% 经验 PDF
scatter(axD, bin_centers2, counts2, msz, ...
    'MarkerFaceColor','none', ...
    'MarkerEdgeColor',[0 0 1], ...
    'DisplayName','Empirical');

% 理论曲线
plotStyled(axD, x6,     y6,     'Tanemura',        lw);
plotStyled(axD, x7,     y7,     'Kiang',           lw);
plotStyled(axD, x8,     y8,     'Ferenc and Neda',          lw);
plotStyled(axD, x_3d,   y_3d,   'Proposed (TG)', lw);
plotStyled(axD, x2_3d,  y2_3d,  'Proposed (GG)', lw);
...
xlabel(axD,'$v$','Interpreter','latex','FontSize',fs_label);
ylabel(axD,'$f(v)$','Interpreter','latex','FontSize',fs_label);
set(axD,'TickDir','in','LineWidth',1);
axD.GridAlpha = 0.15;
